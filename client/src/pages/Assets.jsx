  ...
  const columns = useMemo(() => [
    {
      ...
      width: 160,
      render: (v) => (
        <span style={{ fontFamily: "var(--font-condensed)", fontSize: 13, color: "#111827", fontWeight: 700 }}>
          {v}
        </span>
      ),
    },
      ...
      render: (_, row) => (
        <div className={styles.assetCell}>
          ...
          <div className={styles.assetText}>
            ...
            <span className={styles.assetCategory}>{row.category?.toUpperCase()}</span>
          </div>
        </div>
      ),
    },
    {  
      ...
      render: (_, row) => (
        <div className={styles.assetCell}>
          {/* <img src={getCategoryImage(row.category)} alt="asset" className={styles.assetImg} /> */}
          <div className={styles.assetText}>
            <span className={styles.assetModel}>{row.assigned_to || "—"}</span>
            <span className={styles.assetCategory}>{row.emp_id?.toUpperCase()}</span>
          </div>
        </div>
      ),
    },
    ...
    { key: "to_date", label: "Surrender Date" },
    {
      ...
      label: "Status",
      render: (v) => (
        <span className={`${styles.verifiedBadge} ${v ? styles.yes : styles.no}`}>
    ...
    <div className={styles.parentPage}>
      ...
      <div className={styles.page}>
        ...
        <div className={styles.header}>
          ...
          <button className={styles.addBtn} onClick={() => navigate("/assets/add")}>
            + Add Asset
          </button>
        </div>
      ...
      <EditAssetModal
        ...
        onSaved={() => { fetchAssets(); }}   // refetch list after save
        assetRef={editTarget}
      />
    </div>
  );
